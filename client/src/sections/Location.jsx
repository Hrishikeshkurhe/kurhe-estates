  ...
  return (
    <section className="split rev" id="location">
      <div className={`split-media ${mediaClass}`} ref={mediaRef}>
        <img src="/assets/location-map.jpg" alt="Infinia location map, Amravati" />
      </div>
      <div className={`split-copy ${copyClass}`} ref={copyRef}>
        <p className="eyebrow">05 — Location</p>
        <h2>Plot No. 8,<br />Ganesh Vihar No. 2</h2>
        <p>Near Jagadare Layout, Amravati 444604 — inside a settled residential pocket, minutes from schools, hospitals and the city's daily rhythm.</p>
        <ul className="connect-list">
          {infinia.connectivity.map(([place, dist]) => (
            <li key={place}><span>{place}</span><em>{dist}</em></li>
          ))}
        </ul>
      </div>
    </section>
